...
//			environment.evictMemory();
    ...
    @Override
    public void delete(String tableName, int pidToDelete, DatabaseSession databaseSession) throws BimserverLockConflictException {
        byte[] startSearchingAt = BinUtils.intToByteArray(pidToDelete);
        try (Cursor cursor = initCursor(tableName, databaseSession)) {
            DatabaseEntry keyEntry = new DatabaseEntry(startSearchingAt);
            while (cursor.getSearchKeyRange(keyEntry, null, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
                long id = BinUtils.readInt(keyEntry.getData(), 0);
                if (id == pidToDelete) {
                    cursor.delete(new WriteOptions().setTTL(1));
                } else
                    return;
            }
        } catch (LockConflictException e) {
    ...
    @Override
    public void deleteProj(String tableName, long pidToDelete, DatabaseSession databaseSession) throws BimserverLockConflictException {
        try (Cursor cursor = initCursor(tableName, databaseSession)) {
            DatabaseEntry keyEntry = new DatabaseEntry();
            while (cursor.getNext(keyEntry, null, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
                ByteBuffer buffer = ByteBuffer.wrap(keyEntry.getData());
    ...
    private Cursor initCursor(String tableName, DatabaseSession databaseSession) throws BimserverDatabaseException {
        TableWrapper tableWrapper = getTableWrapper(tableName);
        final CursorConfig cursorConfig = new CursorConfig();
        cursorConfig.setReadUncommitted(true);
        Cursor cursor = tableWrapper.getDatabase().openCursor(getTransaction(databaseSession, tableWrapper), cursorConfig);
